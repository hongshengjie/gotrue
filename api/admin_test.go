...
func (ts *AdminTestSuite) makeSystemUser() string {
	u := models.NewSystemUser(0, ts.Config.JWT.Aud)
...
func (ts *AdminTestSuite) TestAdminUserGet() {
	...
	// Setup request
	w := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodGet, fmt.Sprintf("/admin/users/%d", u.ID), nil)
...
func (ts *AdminTestSuite) TestAdminUserUpdate() {
	...
	// Setup request
	w := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodPut, fmt.Sprintf("/admin/users/%d", u.ID), &buffer)
